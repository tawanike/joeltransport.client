...
const addedServices = [
    ...
    { id: 8, title: "Free Gauteng removal of unused furniture, appliances, clothing & linen", value: "gauteng_removal" },
    { id: 9, title: "International moving services", value: "international_moving_services" },
...
const AddedServices: FC<IProps> = ({ }) => {
    ...
    const handleSelect = (item: any) => {
        bookingDispatch({ type: ADD_FORM_VALUES, payload: { [item.target.value]: item.target.checked } });
    };

    return <>
        <Row className="InventoryItem mb-4">
            {addedServices.map((item) => <Col key={item.id} sm={12} className="mt-3">
                <Form.Check
                    type="checkbox"
                    label={item.title}
                    id={String(item.id)}
