...
const DisplayInvemtorySection: FC<IProps> = ({ room, inventoryItems }) => {
    const [roomInventoryItems, setRoomInventoryItems] = useState<IInventoryItem[]>([]);
    const { state } = useContext(BookingContext);

    useEffect(() => {
        setRoomInventoryItems(inventoryItems.filter((item) => item.category === Number(room.id)));
    }, [inventoryItems]);
    ...
    return <Accordion.Item eventKey={room.id} >
        <Accordion.Header className="InventoryForm__display col-12">
            <div className="row w-100">
                ...
                <div className="col-4 InventoryForm__display__count">
                    <p> Items count({state.inventoryList.filter(x => x.room === Number(room.id)).map(x => x.quantity).reduce(getTotals, 0)})</p>
                </div>
            </div>
        </Accordion.Header>
        <Accordion.Body>
